...
mensaje = "Esta practica de multimedia demuestra el uso de esteganografia LSB en una imagen BMP de 200x200 pixeles. El objetivo es ocultar informacion dentro de los bits menos significativos de los colores sin modificar la apariencia visual de la imagen. El proceso incluye lectura de la estructura del archivo, modificacion de pixeles, almacenamiento de la imagen resultante y recuperacion del mensaje. Tambien se analiza la capacidad de almacenamiento, la integridad del texto y la calidad visual usando PSNR."*10
print(len(mensaje.encode('utf-8')))

# ====== Prueba (misma lógica, solo nombres consistentes) ======
embed_lsb('Images/hongo.bmp', 'hongo_500bytes.bmp', mensaje)
recuperado = extract_lsb('hongo_500bytes.bmp')  # <- era 50000, por eso fallaba
